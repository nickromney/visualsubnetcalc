...
test.describe("Copy Table to Clipboard", () => {
  ...
  test("should copy table with parent network row", async ({
    page,
    browserName,
  }) => {
    test.skip(
      browserName === "firefox",
      "Clipboard API not supported in Firefox tests"
    );
    // Set up a network
    ...
    expect(clipboardContent).toContain(
      "Network Address\tIP\tCIDR\tMask\tType\tRange of Addresses\tUsable IPs\tHosts\tNote"
    );
  ...
  test("should copy table with additional columns when visible", async ({
    page,
    browserName,
  }) => {
    test.skip(
    ...
    expect(clipboardContent).toContain("Parent Network");
    expect(clipboardContent).toContain(
      "192.168.1.0/25\t192.168.1.0\t/25\t255.255.255.128"
    );
    expect(clipboardContent).toContain(
      "192.168.1.128/25\t192.168.1.128\t/25\t255.255.255.128"
    );
  });

  test.skip("should not copy when no network is calculated", async ({
    page,
    browserName,
  }) => {
    test.skip(
  ...
  test("should handle single host networks correctly", async ({
    page,
    browserName,
  }) => {
    test.skip(
  ...
  test("should not duplicate parent network when only one subnet exists", async ({
    page,
    browserName,
  }) => {
    test.skip(
