...
test.describe('Mirror Network Generation', () => {
    ...
    test('should handle no network gracefully', async ({ page }) => {
        ...
        await expect(hint).toContainText('Enter the base network for the mirror (must use same CIDR size as original - /16)');
    });
    ...
    test('should generate mirror with same CIDR sizes', async ({ page }) => {
        ...
        const firstNote = await page.locator('input[data-subnet="10.200.0.0/18"]').inputValue();
        expect(firstNote).toBe('Web Tier');

        const secondNote = await page.locator('input[data-subnet="10.200.64.0/18"]').inputValue();
        expect(secondNote).toBe('App Tier');

        const thirdNote = await page.locator('input[data-subnet="10.200.128.0/17"]').inputValue();
