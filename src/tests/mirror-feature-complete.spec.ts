...
test.describe('Mirror Network Feature - Complete Tests', () => {
    ...
    test('should replace source with mirror', async ({ page }) => {
        ...
        const firstSubnet = await page.locator('.row_address').first().textContent();
    ...
    test('should validate on modal open with pre-filled value', async ({ page }) => {
        await page.click('#generateMirror');
        await page.waitForTimeout(500);
        ...
        if (suggestedValue) {
            ...
            const hasValidation = await mirrorInput.evaluate(el =>
                el.classList.contains('is-valid') || el.classList.contains('is-invalid')
            );
